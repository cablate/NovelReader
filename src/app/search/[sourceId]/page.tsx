...
export default function CategoryList({params}: {params: {sourceId: string}}) {
  ...
  const {data: categoryData, isLoading: isCategoryLoading} = useQuery(["category", sourceId], ({signal}) =>
    api
      .get(`/category?sourceid=${sourceId}`, {signal})
      .then((res) => res.data)
      .then((data) => {
        return data.data;
      }),
  ...
  useEffect(() => {
    ...
    setPageName(categoryData.pageName);
  }, [categoryData]);

  return (
    <div className="flex flex-col w-full overflow-auto">
      {isCategoryLoading && <CircularProgress className="m-auto" />}
      {!isCategoryLoading && arrCategory?.length == 0 && <div className="m-auto text-gray-500">目前沒有資料</div>}
      {!isCategoryLoading &&
